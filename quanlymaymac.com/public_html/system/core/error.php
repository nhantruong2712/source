...
// Errors
// ---------------------------------------------------------------------------
function sa_error($level,$message,$file='current file',$line='(unknown)')
{
	$fatal = false;
	$exception = false;
	...
	if($fatal)
	{
		...
		{
			echo 'SA could not locate error file at '.APPLICATION.'/'.config::get('folder_errors').'/fatal.php';
		}
	...
	elseif($exception)
	{
		...
		{
			echo 'SA could not locate exception file at '.APPLICATION.'/'.config::get('folder_errors').'/exception.php';
		}
	...
	elseif(DEBUG)
	{
		...
		{
			echo 'SA could not locate error file at '.APPLICATION.'/'.config::get('folder_errors').'/nonfatal.php';
		}
	}
...
// Exceptions
// ---------------------------------------------------------------------------
function sa_exception($ex)
{
	sa_error('exception',$ex->getMessage(),$ex->getFile(),$ex->getLine());
