...
class bootstrap
{
	...
	{	
		...
		// Check for GET __hth
		$url = (input::get('__hth')) ? input::get('__hth') : $url;
	...
	public static function run()
	{
		...
		// Load core files
        require_once(SYSTEM.'/core/utils.php');
        ...
        // Autoload classes
		load::autoload();		
        // PreLoad library db and user
        load::library('db');
        load::library('user');

        // Language
        load::language(config::get('language'));
        
        // Theme         
		core::$theme=new theme();
		
		set_error_handler('sa_error');
		set_exception_handler('sa_exception');
