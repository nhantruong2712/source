...
class db
{
	...
	public static function add_connection($name)
	{
		...
		{
			sa_error(E_USER_ERROR,"DB Connection Settings For '$name' Not Found.");
			return FALSE;
		}
	...
	// Query
	// ---------------------------------------------------------------------------
	public static function query($sql, $orm=FALSE)
	{
		return self::$connections['default']->query($sql, $orm);
	}
...
/**
 * SQL Query Building Class For SA Framework DB Drivers
 *
 * @author          Hoang Trong Hoi
 * @copyright       2011
 */
...
class SASQL
{
	...
	static function build_where($where_list,$driver)
	{
		...
		foreach($where_list as $i)
		{
			if($i['type'] == 'column')
			{
				...
				}elseif($i['operator'] == 'IN' || $i['operator'] == 'NOT IN')
				{
					$sql .= " $tick{$i['column']}$tick {$i['operator']} ({$i['value']})";
				}
				else
    ...
    // Fix bug 02/06/2012: select COUNT(*)
	// ---------------------------------------------------------------------------
	...
	static function build_update($query,$driver)
	{
		...
		$sql .= SASQL::build_where($query->where_list,$driver);
	...
	static function build_select($query,$driver)
	{
		...
		// Columns to select
		$sql .= SASQL::build_columns($query->columns,'select',$driver);
...
/**
 * Query Class For SA Framework DB Drivers
 *
 * @author          Hoang Trong Hoi
 * @copyright       2011
 */
...
class SAQuery
{
	...
	public function clause($c)
	{
		...
		if($c != 'AND' AND $c != 'OR')
		{
			throw new SAException("mysql error: The WHERE clause '$c' is not recognized.");
		}
	...
	// Execute
	// ---------------------------------------------------------------------------
	public function execute($model=false) //add model 04/12/2020
	{
		return $this->table->execute($this, $model);
	}
}