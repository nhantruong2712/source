 ...
 * [http://php.net/json json extension] on servers with PHP < 5.2, or 
...
class json
{
	 ...
	 * @var array
	 */
	private static $control_character_map = array(
		'"'   => '\"', '\\' => '\\\\', '/'  => '\/', "\x8" => '\b',
		"\xC" => '\f', "\n" => '\n',   "\r" => '\r', "\t"  => '\t'
	);
	 ...
	 * @var array
	 */
	private static $next_values = array(
		...
		if (is_string($value)) {
			...
			foreach ($char_array as $char) {
				if (isset(self::$control_character_map[$char])) {
					...
					$output .= '\u' . substr(strtolower(utf8::ord($char)), 2);
				}
			}
			$output .= '"';
