...
class http
{
	...
	public static function getContent($URL, array $cURLoptions = null)
	{
		...
		if(!function_exists('curl_init')) throw new SAException('This method requires cURL (http://php.net/curl), it seems like the extension isn\'t installed.');

		// set options
		$options[CURLOPT_URL] = (string) $URL;
		$options[CURLOPT_USERAGENT] = 'SA HTTP v.10';
		...
		// validate
		if($errorNumber != '') throw new SAException($errorMessage);
	...
	public static function redirect($URL, $code = 302, $delay = null)
	{
		// redefine url
		$URL = (string) $URL;
		$code = input::getValue($code, array(301, 302), 302, 'int');
	...
	{
		// header already sent
		if(self::isSent()) throw new SAException('Headers were already sent.');
