...
function saveCachePrints(name,data){
    ...
    }catch(e){console.log('error save localStorage')}
}
...
function findchild(pid){
    ...
    for(var ii in products){ 
        pro = products[ii]; if(pro===null) continue;
...
function checknum(val, val2, zero){
    ...
    try{
        ...
        if(val2>0){
            ...
            val4 = rtrim(rtrim(val3.toFixed(val2+2),"0"),"."); 
            val5 = explode('.',val4);
            if(val5[1]==undefined){
...
function _convertProducts(dataItem,notname){                        
    ...
    for(var i = dataItem.products.length-1;i>=0;i--){
        ...
        var _doingLoadBranches = false;
        function loadJsonBranches(lo){
            
            console.log('loadJsonBranches');
            
            if(_doingLoadBranches) return;
            _doingLoadBranches = true;
            ...
            $.getJSON( "ajax.php?action=branches", function( data ) {
                  ...
                  console.error('Error load branches')
                  loadJsonBranches(lo)
            })
            .always(function() {                 
                _doingLoadBranches = false;
            });
        }
...
var do1do2 = function(){  console.log('schema2:',JSON.stringify(schema1),dbver)
...
var do1do22 = function(){ console.log('schema1:',JSON.stringify(schema1),dbver)
...
//03/29/2020
function _forIndexDB(){ //console.log('_forIndexDB',db.isOpen())
    
    if(db["PriceBook"]) return;
    ...
    if(dbver==1){
        schema1 = {}
        schema1['Books'] = 'id,code,branch';
        if(db.isOpen()) db.close();  
        db.version(2).stores(schema1)
        schema1 = {}
        
        schema1.News = 'id,code';
        schema1.NewsCategory = 'id,code';          
        db.version(3).stores(schema1)            
    }else if(dbver==2){                       
        schema1 = {}
        if(!db['Books'])
        schema1['Books'] = 'id,code,branch';
    ...
    //Invoices Returns PurchaseOrder PurchaseReturns DamageItems CashFlow
    ...
    schema1.NewsCategory = 'id,code';   
    //db.close();   
    schema1.Products = 'id,code,branch';
    schema1.ProductsCategory = 'id';   
    
    schema1.Invoices = 'id,code,branch';
    schema1.Returns = 'id,code,branch';
    schema1.PurchaseOrder = 'id,code,branch';
    schema1.PurchaseReturns = 'id,code,branch';
    schema1.DamageItems = 'id,code,branch';
    schema1.CashFlow = 'id,code,branch';
    
    schema1.Customers = 'id,code,branch';
    schema1.CustomersGroup = 'id,branch';
    
    schema1.Suppliers = 'id,code,branch';
    schema1.SuppliersGroup = 'id,branch';
    
    db.version(4).stores(schema1)      //;console.log('_forIndexDB5',db.isOpen())  
    
    //StockTakes PriceBook
    schema1.StockTakes = 'id,code,branch';
    schema1.PriceBook = 'id,branch';
        
    db.version(5).stores(schema1) 
}
...
$.fn.select3 = function(ops){
    ...
    $(this).each(function(i,v){
        ...
        var _afterSearch = function(se,se2){ console.log('se:',se,se2,sources)
            $('body>.k-animation-container>.k-list-container ul').html('')
            var z = $(v)
            for(var i in sources){
                var check = true
                if(se!=''){
                    check = sources[i].toLowerCase().indexOf(se.toLowerCase())>=0
                } console.log('se check:',check)
                if(check){
                    var lk = $('<li tabindex="-1" role="option" unselectable="on" class="k-item ng-scope '+
                        (i==z.val()?'k-state-selected k-state-focused':'')+'" data-offset-index="'+
                        i+'" id="'+i+'">'+sources[i]+'</li>');
                    lk.hover(function(e){
                    ...
                    lk.click(function(e){                         
                        z.val($(this).attr('id'))
                        z.trigger('change')
                        z.prev().find('span.k-input').html(sources[$(this).attr('id')])
                        $(this).siblings().removeClass('k-state-selected k-state-focused')
                        $(this).addClass('k-state-selected k-state-focused')
                    })
                    $('body>.k-animation-container>.k-list-container ul').append(lk)
                }
                if($('body>.k-animation-container>.k-list-container ul li').length>=ops.limit) break;
            }
        }
        ...
        //03/19/2014    
        if(ops.search){
            if($('body>.k-animation-container .k-list-filter').length==0){
                $('body>.k-animation-container .k-list-container').prepend('<span class="k-list-filter ng-hide"><input class="k-textbox" role="listbox" aria-haspopup="true" aria-expanded="false" aria-owns="" tabindex="0" aria-disabled="false" aria-readonly="false"><span unselectable="on" class="k-icon k-i-search">select</span></span>');                                
            }
        }     
        ...
        function _for(){ //alert($(v).width())
            ...
            $('body>.k-animation-container ul').hover(function(e){
                ...
                $('body>.k-animation-container').hide() 
                $('body>.k-animation-container .k-list-filter').addClass('ng-hide')
            })
        }
        
        $(v).parent().unbind('click')
        $(v).parent().click(function(e){ //console.log('mmm1')
            ...
            //03/19/2020
            var that = $(this)
            if(ops.search){ //console.log('mmm2')
                $('body>.k-animation-container .k-list-filter').removeClass('ng-hide')
                
                var se2 = $('body>.k-animation-container>.k-list-container .k-textbox')
                var se = se2.val()
                ...
                $('body>.k-animation-container').hover(function(e){},function(e){
                    $('body>.k-animation-container,body>.k-animation-container>.k-list-container').hide()
                })
                
                $('body>.k-animation-container,body>.k-animation-container>.k-list-container').show()
                $('body>.k-animation-container').css({
                    left: that.offset().left,
                    top: that.offset().top + that.height()                     
                })
                $('body>.k-animation-container>.k-list-container').css({                     
...
Object.defineProperty(Array.prototype, 'delete', {
    ...
    writable: false,
    value: function(b) {
       var q = this.indexOf(b) 
       if(q>=0)
            return this.splice(q,1), this;
...
function _convertProduct(){
    ...
    for(var i in products){
        p[products[i].id] = products[i] //fixed 05/25/2020
    }
    return p
}
...
//  return qr.createSvgTag();
  return qr.createImgTag(2*3,1*3);
};

//0945535689 75x20 nam 15m
function loadJsonProducts(lo){
    
    console.log('loadJsonProducts');
    ...
    /*$.getJSON( "ajax.php?action=products&branch="+cb, function( data ) {
          products = data;
          try{
          cache_products.setItem("products",data,{expirationAbsolute: null,
                         expirationSliding: 3600,
                         priority: Cache.Priority.HIGH,
                         callback: function(k, v) { console.log('Cache removed: ' + k); }
                        });
    ...
    /*$.ajax({
        ...
        success: function(data){
            ...
            if(data && data.length){  
                ...
                db.table("Products").clear().then(function(){
                    data = data.map(function(x){
                        return $.extend({branch: cb},x,{id:x.id-0})
                    })   
                    db.Products.bulkPut(data).then(function(lastKey) {
              ...
              if(lo && lo.call) lo.call(null,data)
    ...
    */
    ...
    //loadJsonProductsPage(0,lo) 
    loadRevision(cb,function(){
        loadJsonProductsPage(0,lo) 
    })
} 

function makeNullArray(n){
    return "0".repeat(n).split('').map(function(a){return null});
}
function makeRange(start,num){
    var t = makeNullArray(num)
 
    var _ = start;
 
    return t.map(function(a){return _++;})
}

var TotalProducts = {},hashPage;
//$.md5(JSON.stringify($.extend({},filter,{ctg})))
function getHashProducts(){
    return $.md5(JSON.stringify($.extend({},filter,{ctg})))
}

function _productsComplete(){ //console.log('_productsComplete x:',products,cfs,cb)
    /*console.log('_productsComplete:',products.length,localStorage['revision_products_'+cfs.id+'_'+cb+'_'+
    localStorage['revision_products_'+cfs.id+'_'+cb+'']+'_total'])*/
    
    if(localStorage['revision_products_'+cfs.id+'_'+cb+'']==undefined)
        return console.log('ggg 1'),0;
    
    var q = products.filter(function(a){return a==null}).length==0 &&     
           
    localStorage['revision_products_'+cfs.id+'_'+cb+'_'+
    localStorage['revision_products_'+cfs.id+'_'+cb+'']+'_total']==products.length;
    
    //console.log('_productsComplete:',q)
    
    return q;
}

//db.Products.toCollection().reverse().sortBy('id')
function loadJsonProductsPage(currentItem,lo){
    
    console.log('loadJsonProductsPage',_productsComplete)
    
    if(_productsComplete()) 
        return (lo && lo.call && lo.call(null));
        
    console.log('loadJsonProductsPage 1')
    
    var ps = localStorage['revision_products_'+cfs.id+'_'+cb+'_'+
    localStorage['revision_products_'+cfs.id+'_'+cb+'']+'_pages'];
    
    console.log('loadJsonProductsPage 2')
    
    if(ps!=undefined){
        ps = ps.split(',');
        while(ps.indexOf(currentItem/10+'')>=0){
            currentItem+=1000;
        }
    }
    
    console.log('loadJsonProductsPage 3');
    
    $.ajax({
        url: '/ajax.php?action=products&branch='+cb+'&pageSize=1000&sell=2&currentItem='+currentItem,
        dataType: 'JSON',
        success: function(data){
            var qqq = false;
            if(data && data.total){  
                
                //TotalProducts[getHashProducts()] = data.total-0
                //{"type":[1,2,3],"ShowInActiveProduct":1,"keywordParam":"","onhandFilter":0,"Expire":0,"range":"","user":0,"ctg":null}
                //$.md5(JSON.stringify($.extend({},filter,{ctg})))
                //"59ba016c696865893dc7f24ece8b1381"
                TotalProducts['59ba016c696865893dc7f24ece8b1381'] = data.total-0
                /*localStorage['revision_products_'+cfs.id+'_'+cb+'_'+
                    localStorage['revision_products_'+cfs.id+'_'+cb+'']] = 
                    {
                        total:data.total-0, 
                        pages:[0,1,null,....,3722-1]
                    }*/
                ...
                localStorage['revision_products_'+cfs.id+'_'+cb+'_'+
                    localStorage['revision_products_'+cfs.id+'_'+cb+'']+'_total'] = data.total;
                
                var _u = localStorage['revision_products_'+cfs.id+'_'+cb+'_'+
                    localStorage['revision_products_'+cfs.id+'_'+cb+'']+'_pages'];
                if( _u == undefined)
                    _u = makeNullArray(data.total-0);
                else _u =_u.split(',')
                    
                for(var i=0;i<Math.ceil(data.data.length/10);i++){
                    var j = currentItem/10;
                    _u[j+i] = j+i;                     
                }  
                 
                localStorage['revision_products_'+cfs.id+'_'+cb+'_'+
                    localStorage['revision_products_'+cfs.id+'_'+cb+'']+'_pages'] = _u;
                
                if(currentItem==0){
                    products = makeNullArray(data.total-0)
                    //products = data.data;  
                    Array.prototype.splice.apply(products,[currentItem,data.data.length].concat(data.data))
                    
                }
                  
                else{
                    //products = products.merge(data.data) 
                    Array.prototype.splice.apply(products,[currentItem,data.data.length].concat(data.data))
                }
                console.log('products.length',products.length)
                
                var kk = function(){ console.log('data.data.length',data.data.length)
                    data.data = data.data.map(function(x){
                        return $.extend({branch: cb},x,{id:x.id-0})
                    })   
                    db.Products.bulkPut(data.data).then(function(lastKey) {
                         console.log('lastKey:',lastKey)
                    }).catch(Dexie.BulkError, function (e) {                
                        console.error ("Some raindrops did not succeed. However, " +
                           data.data.length-e.failures.length + " raindrops was added successfully");                                                         
                    }); 
                    /*for(var m in data.data){
                        db.Products.get(data.data[m].id).then(function(a){
                            if(a) db.Products.update(data.data[m].id, a).catch(Dexie.DexieError, function (e) {                
                                console.error ("1");                                                         
                            })
                            else db.Products.add(data.data[m]).catch(Dexie.DexieError, function (e) {                
                                console.error (e);                                                         
                            })
                ...
                if(currentItem==0)
                    db.table("Products").clear().then(kk)
                else 
                    kk()
                ...
                if(data.data.length + currentItem<data.total){
                    currentItem += 1000;
                    loadJsonProductsPage(currentItem,lo)
                }
             ...
             if(qqq && lo && lo.call) lo.call(null,data)
...
function loadRevision(cb,lo){ console.log('loadRevision',cb,cfs)
    if(!cb){
    ...
    $.ajax({
        url: '/data/'+cfs.id+'/revision_'+cb+'.json?'+(new Date().getTime()),
        dataType: 'json',
        success: function(data){
            
            //{"revision":{"products":1586503512000}}
            //localStorage.revision_products_{cfs.id}_{cb}
            
            if(localStorage['revision_products_'+cfs.id+'_'+cb+''] != data.revision.products){
                /*                
                delete localStorage['revision_products_'+cfs.id+'_'+cb+'_'+
                    localStorage['revision_products_'+cfs.id+'_'+cb+'']+'_total'];
...
function loadProductsFromCache(lo){
    
    console.log('loadProductsFromCache');
    
    if(!cb){
        setTimeout(function(){
            loadProductsFromCache(lo)
        },1000);
        return;
    }
    //db.Products.toCollection().reverse().sortBy('id')
    db.open().then(function(){
        db.table("Products") .reverse()     //.sortBy('id')  
        .toArray().then(function(x){  
            products = x;         
        }).catch(function(){
            products = [];
        }).finally(function(){
            lo && lo.call && lo.call(null,products)
        })
    }).catch(function(e){
        console.log('e3:',e)
...
//11/18/2020
//1 ban 3 nhap 5 tra ncc 7 kiem 10 xuat huy
//11 tra hang
//13 doi hang (chua lam)
//15 chuyen hang, 16 san xuat (chua lam)
var invoiceTypes = {
    1: 'Bán hàng',
    3: 'Nhập hàng',
    5: 'Trả NCC',
